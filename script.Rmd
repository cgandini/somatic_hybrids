---
title: "hybrid recombination"
output: html_notebook
---
### Get recombination regions
...
```{r}
  ...
  ### ALL Breakpoints plot
  
  
breakpoints_in_3 = read.delim(file="../plot_hotspots/recombination_breakpoints_in_three.txt", header=T) 
breakpoints_in_po = read.delim(file="../plot_hotspots/ntpoA01_po_breakpoints.txt", header=T) 
nthn_regions = read.delim(file="../plot_hotspots/nthn_regions.txt", header=F)
ntpo_regions = read.delim(file="../plot_hotspots/ntpo_regions.txt", header=F)
overlapping_regions = read.delim(file="../plot_hotspots/overlapping_hr_only.txt", header=F)
recombining_ntpo = read.delim(file="../plot_hotspots/ntpo_recombining.txt", header=F)
recombining_nthn = read.delim(file="../plot_hotspots/nthn_recombining.txt", header=F)
recombining_nt_nthn = read.delim(file="../plot_hotspots/nt_nthn_recombining.txt", header=F)
recomb_windows = read.delim(file="../plot_hotspots/recomb_windows_5kb.txt", header=T)
nt_LR = read.delim(file="../plot_hotspots/nt_LR.bed", header=F)
po_LR = read.delim(file="../plot_hotspots/po_LR.bed", header=F)
nt_SR = read.delim(file="../plot_hotspots/nt_SR.bed", header=F)
po_SR = read.delim(file="../plot_hotspots/po_SR.bed", header=F)
  
df_breakpoints = data.frame(hybrid=character(), event=character(), reads=numeric(), coord=numeric())
  
for(i in 1:dim(breakpoints_in_3)[1]) {
  hybrid=breakpoints_in_3$somatic.hybrid[i]
  recombination_event=breakpoints_in_3$Recombination.event[i]
  reads=breakpoints_in_3$supporting.PE.reads[i]
  start=breakpoints_in_3$nt.breakpoint.start[i]
  end=breakpoints_in_3$nt.breakpoint.end[i]
  
  df_breakpoints = rbind(df_breakpoints, c(hybrid, NA, 0, start))
  df_breakpoints = rbind(df_breakpoints, c(hybrid, recombination_event, reads, (start+end)/2))
  df_breakpoints = rbind(df_breakpoints, c(hybrid, NA, 0, end))
  
}

colnames(df_breakpoints)= c("hybrid", "event", "reads", "coord")
df_breakpoints$coord = as.numeric(df_breakpoints$coord) 
df_breakpoints$reads = as.numeric(df_breakpoints$reads) 
df_breakpoints = df_breakpoints %>% arrange(hybrid, coord)

library(ggplot2)
library(scales)

plot_all_breakpoints=ggplot(df_breakpoints) +
  aes(y=reads, x=coord, group=hybrid, colour=hybrid) +
  geom_line(size = 0.5) +
  scale_x_continuous(limits = c(1, p1_length), labels = label_number(), expand = c(0, 0)) +
  scale_y_continuous(limits = c(-12,100)) +
  theme_minimal() +
  geom_segment(data=p1_genes, aes(x = start, xend=end, y = -3, yend = -3), size=3, color="gray42") +
  geom_segment(data=ntpo_regions, aes(x = V2, xend=V3, y = -6, yend = -6), size=3, color="gray") +
  geom_segment(data=recombining_ntpo, aes(x = V1, xend=V2, y = -6, yend = -6), size=3, color="red") +
  geom_segment(data=nthn_regions, aes(x = V2, xend=V3, y = -9, yend = -9), size=3, color="gray") +
  geom_segment(data=recombining_nthn, aes(x = V1, xend=V2, y = -9, yend = -9), size=3, color="red") +
  geom_segment(data=nthn_regions, aes(x = V2, xend=V3, y = -12, yend = -12), size=3, color="gray") +
  geom_segment(data=recombining_nt_nthn, aes(x = V1, xend=V2, y = -12, yend = -12), size=3, color="red") +
  geom_rect(data=recomb_windows, aes(ymin=0, ymax=100, xmin=start,xmax=end, fill=PE.reads), alpha =0.5) 

pal = c("#6DC8BF","#FFF200", "#F15A22")
plot_windows = ggplot()+
  geom_rect(data=recomb_windows, aes(ymin=0, ymax=100, xmin=start,xmax=end, fill=PE.reads), alpha =0.5) +
  scale_x_continuous(limits = c(1, p1_length), labels = label_number(), expand = c(0, 0)) +
  scale_y_continuous(limits = c(-12,100)) +
...
ggsave(file="/Users/carolinagandini/OneDrive/Lab_Files/Hybrids/ntpo/ntpoa01/recombination_analysis/plot_hotspots/hotspots.pdf", plot = plot, device = NULL, path = NULL,
  scale = 1, width = 20, height = 5, units = "in",
  dpi = 1200, limitsize = F)   
  
### Plot Recombinations only in ntpoA01


plot_nt_cov=ggplot(depth_cov %>% filter(parental == "nt_mtDNA")) + aes(x= coord, y=depth) +
  geom_area(fill="lightgray") +
  scale_x_continuous(limits = c(1, p1_length), labels = label_number(), expand = c(0, 0)) +
  scale_y_continuous(limits = c(0,200)) +
  theme_void()

plot_po_cov=ggplot(depth_cov %>% filter(parental == "po_mtDNA")) + aes(x= coord, y=depth) +
  geom_area(fill="lightgray") +
  scale_x_continuous(limits = c(1, p2_length), labels = label_number(), expand = c(0, 0)) +
  scale_y_continuous(limits = c(0,200)) +
  theme_void()

plot_nt_breakpoints=ggplot(df_breakpoints %>% filter(hybrid == "ntpoA01")) +
  aes(y=reads, x=coord) +
  geom_line(size = 0.5) +
  scale_x_continuous(limits = c(1, 430597), labels = label_number(), expand = c(0, 0)) +
  scale_y_continuous(limits = c(-9,100)) +
  theme_minimal() +
  geom_segment(data=p1_genes, aes(x = start, xend=end, y = -3, yend = -3), size=3, color="gray42") +
  geom_segment(data=nt_LR, aes(x = V2, xend=V3, y = -6, yend = -6), size=3, color="gray42") +
  geom_segment(data=nt_SR, aes(x = V2, xend=V3, y = -9, yend = -9), size=3, color="gray42")


df_breakpoints_po = data.frame(hybrid=character(), event=character(), reads=numeric(), coord=numeric())
  
for(i in 1:dim(breakpoints_in_po)[1]) {
  hybrid=breakpoints_in_po$somatic.hybrid[i]
  recombination_event=breakpoints_in_po$Recombination.event[i]
  reads=breakpoints_in_po$supporting.PE.reads[i]
  start=breakpoints_in_po$po.breakpoint.start[i]
  end=breakpoints_in_po$po.breakpoint.end[i]
  
  df_breakpoints_po = rbind(df_breakpoints_po, c(hybrid, NA, 0, start))
  df_breakpoints_po = rbind(df_breakpoints_po, c(hybrid, recombination_event, reads, (start+end)/2))
  df_breakpoints_po = rbind(df_breakpoints_po, c(hybrid, NA, 0, end))
  
}

colnames(df_breakpoints_po)= c("hybrid", "event", "reads", "coord")
df_breakpoints_po$coord = as.numeric(df_breakpoints_po$coord) 
df_breakpoints_po$reads = as.numeric(df_breakpoints_po$reads) 
df_breakpoints_po = df_breakpoints_po %>% arrange(hybrid, coord)

plot_po_breakpoints=ggplot(df_breakpoints_po) +
  aes(y=reads, x=coord) +
  geom_line(size = 0.5) +
  scale_x_continuous(limits = c(1, p2_length), labels = label_number(), expand = c(0, 0)) +
  scale_y_continuous(limits = c(-9,100)) +
  theme_minimal() +
  geom_segment(data=p2_genes, aes(x = start, xend=end, y = -3, yend = -3), size=3, color="gray42") +
  geom_segment(data=po_LR, aes(x = V2, xend=V3, y = -6, yend = -6), size=3, color="gray42") +
  geom_segment(data=po_SR, aes(x = V2, xend=V3, y = -9, yend = -9), size=3, color="gray42")
